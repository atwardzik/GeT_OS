...
#define CTRL_KEY(x) (x & 0b00011111)
...
static const struct Line *visual_editor_get_screen_line(
        const struct VisualEditor *editor, const unsigned int line_number
) {
        const int current_line_index = (int) line_number - editor->top_line_number;
...
static bool cursor_can_move_dn(
        ...
        if (check_line_exists(editor->file_editor, cursor->row + count)) {
                return true;
        }
